<template>
  <div class="w-full flex flex-col items-center justify-center py-1 pb-4">
    <h3 class="text-2xl font-semibold mb-2">{{ search }}</h3>
    <div class="text-zinc-800 w-[90%] flex flex-col gap-6">
      ...
      <Paginator
        v-if="productosPage && productosPage.content.length > 0"
        :total-items="productosPage.page"
        :items-per-page="5"
        v-model:current-page="page"
      ></Paginator>
    </div>
    <h3 v-if="productosPage?.content.length === 0" class="text-xl m-4">
      No hay productos
    </h3>
  </div>
</template>
